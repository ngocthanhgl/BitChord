...
class EqScheduleTest {
    ...
    fun `swap progress table matches the spec`() {
        assertEquals(0.35f, EqSchedule.BASS_SWAP_PROGRESS[TransitionType.SMOOTH_CROSSFADE])
        assertEquals(0.30f, EqSchedule.BASS_SWAP_PROGRESS[TransitionType.HARMONIC_BLEND])
    ...
    @Test
    fun `harmonic keeps incoming mids present from entry`() {
        // Keys match: no entry kill, only bass segregated.
        assertEquals(1f, into(TransitionType.HARMONIC_BLEND, 0f).mid)
